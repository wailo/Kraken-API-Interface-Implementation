...
public:
  ...
  */
  ...
  //! mf = free margin = equity - initial margin (maximum margin available to
  open new positions)
  ...
  */
  ...
  boost::
      ...
      //! limitprice = triggered limit price (quote currency, when limit based
      ...
      //! trades = array of trade ids related to order (if trades info requested
      and data available)
      //!
      //! - Note: Unless otherwise stated, costs, fees, prices, and volumes are
      in the asset pair's scale, not the currency's scale. For example, if the
      asset pair uses a lot size that has a scale of 8, the volume will use a
      scale of 8, even if the currency it represents only has a scale of 2.
      Similarly, if the asset pair's pricing scale is 5, the scale will remain
  ...
  //!
  //! - Note: Times given by order tx ids are more accurate than unix
  timestamps. If an order tx id is given for the time, the order's open time is
  used
  //!
  */
  ...
  */
  ...
  exclusive)
  //! end = ending unix timestamp or trade tx id of results (optional.
  inclusive)
  //! ofs = result offset
  //!
  ...
  //!
  //! - If the trade opened a position, the follow fields are also present in
  the trade info:
  ...
  */
  ...
  //!         viqc = volume in quote currency
  //!
  //! - Note: Unless otherwise stated, costs, fees, prices, and volumes are in
  the asset pair's scale, not the currency's scale.
  //!
  */
  ...
  */
  ...
  //!     nextfee = next tier's fee for pair (if not fixed fee.  nil if at
  lowest fee tier)
  //!     nextvolume = volume level of next tier (if not fixed fee.  nil if at
  lowest fee tier)
  //!     tiervolume = volume level of current tier (if not fixed fee.  nil if
  at lowest fee tier)
  ...
  */
  ...
  //! new = whether or not to generate a new address (optional.  default =
  false)
  //!
  ...
  */
  ...
  //!    return = a return transaction initiated by Kraken; it cannot be
  canceled
