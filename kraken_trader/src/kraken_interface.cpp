...
std::string add_standard_order( const std::string& pair,
  ...
  KAPI::Input in;
  in.insert(make_pair("pair", pair));
  in.insert(make_pair("type", type));
  in.insert(make_pair("ordertype", ordertype));
  in.insert(make_pair("price", price));
  in.insert(make_pair("price2", price2));
  in.insert(make_pair("volume", volume));
  in.insert(make_pair("leverage", leverage));
  in.insert(make_pair("oflags", oflags));
  in.insert(make_pair("starttm", starttm));
  in.insert(make_pair("expiretm", expiretm));
  in.insert(make_pair("userref", userref));
  in.insert(make_pair("validate", validate));
  kapi.private_method("AddOrder", in);
}
